class Game {
    constructor (field, btn, score, msg) {
      ...
      for (let i = 0; i < 25; i++) {  // в items хранится инфа по каждой клетке поля, не созданному элементу, а именно клетке
        this.items.push({  // клетки пронумерованы построчно, 1 строка с 0 по 4, 2 строка с 5 по 9 и тд
          id: false,  // если клетка пустая -false, если заполнена - id элемента в этой клетке
      ...
      }  // через абсолютное позиционирование, подробнее в setPosition()
    ...
    // сначала создёт массив со всеми пустыми клетками, если их нет, элемент не создаёт
    ...
    let elem = document.getElementById(item.id);
      elem.textContent = item.value *= 2;  // и this.item.value
    ...
    elem.classList.remove('n' + item.value / 2);
      ...
      if (+item.value >= 2048) this.record();  // победа, показать плашку ШИН, всё заблочить, добавить рекорд
    }
    restart() {
    ...
    /*попытка сделать game over*/
    checkLoser(gameValues) {
      // проверяет, если все поле заполнено, игре конец
      // и пользователь проиграл
      if (
  ...
  back() {
    ...
    record() {
    let list = document.getElementById('raiting');
    ...
    const str = Math.floor(diff / 60).toString().padStart(2, 0) + ':' + Math.floor(diff % 60).toString().padStart(2, 0);
      this.raiting.push( {diff, str} );  // закинуть новый рекорд в массив, отсортировать
    this.raiting.sort((a, b) => a.diff - b.diff);
      this.raiting.forEach(item => list.insertAdjacentHTML('beforeend', `<li>${item.str}</li>`));  // наполняет список заново
